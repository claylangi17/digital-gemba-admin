    ...
    <div class="card shadow-none border border-gray-200 dark:border-neutral-600 dark:bg-neutral-700 rounded-lg h-full bg-gradient-to-r from-cyan-600/10 to-bg-white">
        <div class="card-body p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <p class="font-medium text-neutral-900 dark:text-white mb-1">Total Isu</p>
                    <h6 class="mb-0 dark:text-white"> {{ $total_issue }} </h6>
                </div>
            ...
            <p class="font-medium text-sm text-neutral-600 dark:text-white mt-3 mb-0 flex items-center gap-2">
                @if ($total_issue > $total_issue_last)
                    <span class="inline-flex items-center gap-1 text-danger-600 dark:text-danger-400"><iconify-icon icon="bxs:up-arrow" class="text-xs"></iconify-icon> + {{ $total_issue - $total_issue_last }} </span>
                @else
                    <span class="inline-flex items-center gap-1 text-success-600 dark:text-success-400"><iconify-icon icon="bxs:down-arrow" class="text-xs"></iconify-icon> - {{ $total_issue_last - $total_issue }} </span>
                @endif
        ...
        <div class="card-body p-5">
            ...
            <p class="font-medium text-sm text-neutral-600 dark:text-white mt-3 mb-0 flex items-center gap-2">
                @if ($overdue_action > $overdue_action_last)
                    <span class="inline-flex items-center gap-1 text-danger-600 dark:text-danger-400"><iconify-icon icon="bxs:up-arrow" class="text-xs"></iconify-icon> + {{ $overdue_action - $overdue_action_last }} </span>
                @else
                    <span class="inline-flex items-center gap-1 text-success-600 dark:text-success-400"><iconify-icon icon="bxs:down-arrow" class="text-xs"></iconify-icon> - {{ $overdue_action_last - $overdue_action }} </span>
                @endif
                Dibandingkan Bulan Lalu
            </p>
        </div>
