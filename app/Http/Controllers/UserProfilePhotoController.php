...
class UserProfilePhotoController extends Controller
{
    public function create_api(Request $request)
    {
        try {
            ...
            foreach ($request->file('files', []) as $file) {
                ...
                // Store file
                $path = $file->storeAs(
                    'uploads/user/profile/' . (string) $request->user_id,
                    $filename,
                    'public'
                );
                ...
                // Create record
                $profilePhoto = UserProfilePhoto::create([
            ...
            Log::error('Failed to add Profile Photo via API', ['error' => $e->getMessage()]);
    ...
    public function get_api($user_id)
    {
        try {
            ...
            $record = UserProfilePhoto::where('user_id', $user_id)->orderByDesc('created_at')->first();
            ...
            Log::error('Failed to fetch profile photo via API', ['error' => $e->getMessage()]);
    ...
    public function delete_api($file_id)
    {
        try {
            ...
            $record = UserProfilePhoto::find($file_id);
            ...
            Log::error('Failed to delete profile photo  via API', ['error' => $e->getMessage()]);
